...
if(count($get_vendor_packages) > 0){
      $get_data['vendors_packages'] = $get_vendor_packages;
        ...
        if($validator->fails()){
          ...
          if(Role::insert(
            array(
              ...
              $User->retypepassword     =    bcrypt( trim(Request::input('retypepassword')) );
  ...
  public function userRegistration(){
    if( Request::isMethod('post') && Session::token() == Request::Input('_token')){
      ...
      $messages = [
        'display_name.required' => Lang::get('validation.display_name_required'),
        'user__name.required' => Lang::get('validation.user_name_required'),
...
[
    ...
    'retypepassword'  => 'required|min:6|max:25',
    'secret_key'  => 'required',
]
);
 ...
 $vendor_data['profile_details'] = array(  'display_name' => Request::Input('display_name'),'name' => Request::Input('user_name'),'email' => Request::Input('email'));
  ...
  public function vendorRegistration(Request $request){
    if( Request::isMethod('post') && Session::token() == Request::Input('_token')){
      ...
      $rules = [
        ...
        'vendor_passport'             => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
        'vendor_visa_page'             => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
        'vendor_national_id'             => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:2048',
        't_and_c'                           => 'required',
        'vpkg'                           => 'required',
      ];
      ...
      $messages = [
        ...
        'vendor_passport.required' => Lang::get('validation.vendor_passport'),
        'vendor_visa_page.required' => Lang::get('validation.vendor_visa_page'),
        'vendor_national_id.required' => Lang::get('validation.vendor_national_id'),
        't_and_c.required' => Lang::get('validation.t_and_c'),
        'vpkg.required' => Lang::get('validation.t_and_c')
      ];
      ...
      if($validator->fails()){
        ...
        if (!empty($get_available_roles)) {
          ...
          $Userdetails = new UsersDetail;
          $email_options = get_emails_option_data();
          $User->name=Request::Input('vendor_reg_display_name');
          ...
          if(Request::has('vendor_passport'))
          {
            $User->user_passport=Request::file('vendor_passport')->storeAs('publics/vendor'.'/'.$User->display_name, Request::file('vendor_passport')->getClientOriginalName());

          }
          if(Request::has('vendor_visa_page'))
          {
            $User->user_visa=Request::file('vendor_visa_page')->storeAs('publics/vendor'.'/'.$User->display_name, Request::file('vendor_visa_page')->getClientOriginalName());

          }
          if(Request::has('vendor_national_id'))
          {
            $User->user_national_id=Request::file('vendor_national_id')->storeAs('publics/vendor'.'/'.$User->display_name, Request::file('vendor_national_id')->getClientOriginalName());
          ...
          if ($User->save()) {
            ...
            if ($Roleuser->save()) {
              $get_package  = VendorPackage::where(['id' => Request::Input('vpkg')])->first();
