...
class LoginController extends Controller
{
    ...
    if( Request::isMethod('post') && Session::token() == Request::Input('_token') ){
      ...
      if($validator->fails()){
        ...
        if(!empty($data) && isset($data->password) && isset($data->id)){
          ...
          if(Hash::check(Request::Input('admin_login_password'), $password) && Hash::check(Request::Input('admin_login_password'), $data->password) && isset($get_user_role['user_role_slug']) && ($get_user_role['user_role_slug'] == 'administrator')){
   ...
   public function VendorLogin()
  {
    ...
    if( Request::isMethod('post') && Session::token() == Request::Input('_token') ){
        ...
        $rules = [
                    'vendor_login_email'             => 'required|email',
                    'vendor_login_password'          => 'required'
        ];

        $messages = [
                      'vendor_login_email.required' => Lang::get('validation.email_required'),
                      'vendor_login_email.email' => Lang::get('validation.email_is_email'),
                      'vendor_login_password.required' => Lang::get('validation.password_required')
        ];
        ...
        if($validator->fails()){
          return redirect()-> back()
          ->withInput(Request::except('vendor_login_password'))
          ->withErrors( $validator );
        }
        else{
          $email      =      Request::Input('vendor_login_email');
          $password   =      bcrypt(Request::Input('vendor_login_password'));
          ...
          if(!empty($data) && isset($data->password) && isset($data->id)){
            ...
            if(Hash::check(Request::Input('vendor_login_password'), $password) && Hash::check(Request::Input('vendor_login_password'), $data->password) && isset($get_user_role['user_role_slug']) && ($get_user_role['user_role_slug'] == 'vendor') ){
              ...
              if($remember == TRUE){
                $remember_data = '';
                $remember_data = $data->id . '#' . base64_encode(Request::Input('vendor_login_password'));
