...
class FrontendManagerController extends Controller
{
  ...
  public function productCategoriesSinglePageContent( $params ){
    ...
    $get_cat_product_and_breadcrumb  =  $this->product->getProductByCatSlug($params, array('sort' => $sort, 'price_min' => $price_min, 'price_max' => $price_max, 'selected_colors' => $selected_colors, 'selected_sizes' => $selected_sizes, 'search_string'=>$search_string));
  ...
  public function addtocart($id){
    ...
    $array['order_price'] =      get_product_price_html_by_filter($product->price);
    $array['img_url'] =          $product->img_url;
    $array['variation_id'] =     -1;
    $array['options'] =          array();
    $array['tax'] =              0;
    $array['product_type'] =     get_product_type( $id );
    $array['acces_token'] =      "";
    $array['designer_data'] =    json_encode($designer_data);

    $new = new UserOrderItem();

    $new->order_id = 1;
    $new->order_data = json_encode($array);
