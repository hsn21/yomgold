...
class ProductsController extends Controller
{
  ...
  public function get_categories_by_id($cat_id , $cat_type){
    $categories = array();

    $get_categories = DB::table('terms')
    ->where('terms.term_id', '=', $cat_id)
                      ->where('terms.type', '=', $cat_type)
                      ->where('terms.status', '=', 1)
                      ->join('term_extras', 'terms.term_id', '=', 'term_extras.term_id')
                      ->select(DB::raw('terms.term_id as id'), 'terms.name', DB::raw('terms.slug as slug'), 'terms.type', 'terms.parent', 'terms.status', DB::raw("max(CASE WHEN term_extras.key_name = '_category_description' THEN term_extras.key_value END) as description"), DB::raw("max(CASE WHEN term_extras.key_name = '_category_img_url' THEN term_extras.key_value END) as img_url"), 'terms.created_at', 'terms.updated_at')
  ...
  */
  ...
  public function getProductByCatSlug($cat_slug, $filter = array()){
    ...
    if(!empty($get_term) && isset($get_term->term_id)){
      ...
      if(count($get_child_cat) > 0){
        ...
        foreach($cats_arr as $cat){
          if( (count($filter_arr) > 0 && ($cat['search_type'] == 'color-filter' || $cat['search_type'] == 'size-filter')) || (count($filter_arr) == 0 && $cat['search_type'] == 'category-filter')){
            ...
            $get_post_data->where(['products.status' => 1, 'object_relationships.term_id' => $get_term_data[0]['term_id'] ]);
            ...
            if(isset($filter['search_string'])){
             $get_post_data->where('products.title', 'LIKE', '%'.$filter['search_string'].'%');   
            }
     ...
     if($parent_cat[0]['parent'] > 0){

$parent_cat1 = $this->getTermDataById( $parent_cat[0]['parent'] );

         $str = '<div class="breadcrumb_content"><ul><li ><a href="'. route('home-page') .'"><i class="fa fa-home"></i></a></li><li ><a href="'. route('shop-page') .'">'. Lang::get('frontend.all_products_label' ) .'</a></li><li ><a href="'. route('categories-page', $parent_cat1[0]['slug']) .'">'. $parent_cat1[0]['name'] .'</a></li><li ><a href="'. route('categories-page', $parent_cat[0]['slug']) .'">'. $parent_cat[0]['name'] .'</a></li><li >'. $cat_data['name'] .'</li></ul></div>';
     }else{
        
        $str = '<div class="breadcrumb_content"><ul><li ><a href="'. route('home-page') .'"><i class="fa fa-home"></i></a></li><li ><a href="'. route('shop-page') .'">'. Lang::get('frontend.all_products_label' ) .'</a></li><li ><a href="'. route('categories-page', $parent_cat[0]['slug']) .'">'. $parent_cat[0]['name'] .'</a></li><li >'. $cat_data['name'] .'</li></ul></div>';
      }
      }
      else{
        $str = '<div class="breadcrumb_content"><ul><li ><a href="'. route('home-page') .'"><i class="fa fa-home"></i></a></li><li ><a href="'. route('shop-page') .'">'. Lang::get('frontend.all_products_label' ) .'</a></li><li >'. $cat_data['name'] .'</li></ul></div>';
      }
      ...
      $post_array['select_parent']       =  $cat_data['parent_id'];
      $post_array['cat_id']       =  $cat_data['id'];

$get_cat_image =  DB::table('term_extras')->where(['term_extras.key_name' => '_category_img_url', 'term_extras.term_id' => $cat['id'] ])->first();

            

      $post_array['cat_image']       = $get_cat_image->key_value;
