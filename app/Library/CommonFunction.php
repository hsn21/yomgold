...
class CommonFunction
{
  ...
  public function get_dynamic_frontend_content_data(){
    ...
    $data_ary['shop_by_cat']                 =   $get_cat_tree ;
...
<a href="'. $route .'" class="main-menu__link">
'. $name .'
          ...
          elseif ($menu->status == 'enable' && $label1[0] == 'collection') {
            $html .= '<li class="mega_items">';
            $html .= '<a href="#">'. $name .'</a>';
            $html .= '<div class="mega_menu">';
        ...
        elseif (isset($label2[0]) && $label2[0] == 'cat' && isset($label2[1]) && $menu->status == 'enable') {
          $id_c=$label2[1];
          $get_term = $product->getTermDataById( $id_c );

          $get_categories = $product->get_categories($id_c, 'product_cat');
...
$sub_c='';
if($get_categories[0]['id']==$label2[1]){
$sub_c=$get_categories[0]['children'];
}elseif($get_categories[1]['id']==$label2[1]){
$sub_c=$get_categories[1]['children'];
}


 if(is_array($sub_c) && count($sub_c) >0){
  $i = 0;
                foreach($sub_c as $subCat){
                  $i++;
...
<a class="megamenu-links__item-link" href="'. route('categories-page', $subCat['slug']) .'">'. $subCat['name'] .'</a>
...
$html .= '<ul class="megamenu-links">';
        foreach($subCat['children'] as $subCat1){
  $html .= '<li class="megamenu-links__item"><a class="megamenu-links__item-link" href="'. route('categories-page', $subCat1['slug']) .'">'. $subCat1['name'] .'</a></li>';
}
$html .= '
</ul>';
}
